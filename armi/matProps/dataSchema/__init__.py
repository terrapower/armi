...
* The first function type listed is **"constant."** "constant" has one property, “value,” which can only be a number.
  ...
  armi.matProps directly through the python backend in ConstantFunction.py and ConstantFunction.py.
* **polynomial** has one property: "coefficients." This uses the "patternProperties" keyword to conform all keys of
  ...
  all keys in "polynomial" follow the set regex. Polynomial functions are added to armi.matProps directly through the
...
* **hyperbolic** has one property: "coefficients." “coefficients” has the properties: "hyperbolic function" (string),
  ...
  all required to describe a hyperbolic function. Currently, armi.matProps only support the hyperbolic tangent function.
  Hyperbolic functions are added to armi.matProps directly through the python backend in HyperbolicFunction.py and
  HyperbolicFunction.py.
* **power law** has one property: "coefficients." This has the properties: "exponent" (number), "outer multiplier"
  (number), and "inner adder" (number). These are all required. Power law functions are added to armi.matProps directly
...
* **piecewise** is a recursive function that has one property: "functions." This is an array with two properties; the
  ...
  Piecewise functions are added to armi.matProps directly through the python backend in PiecewiseFunction.py.
* **temperature dependent table** has one required property: "rounding scheme" (number). Temperature dependent table
  functions are added to armi.matProps directly through the python backend in TemperatureDependentTableFunction.py.
* **time and temperature dependent table** and "cycles and temperature dependent table" both follow the same schema.
  ...
  scheme" can only be the values (strings): “log-lin,” and "lin-lin." 2D table functions are added to armi.matProps
...
The final block of schema are the allowed materials properties that are currently in armi.matProps. These use the
