...
def plotFaceMap(
    ...
    if len(data) != len(labels):
        raise ValueError(
            "Data had length {}, but lables had length {}. They should be equal length.".format(len(data), len(labels))
        )
...
def plotAssemblyTypes(
    ...
    if assems is None and blueprints is None:
        raise ValueError("At least one of these inputs must be non-None: blueprints, assems")
