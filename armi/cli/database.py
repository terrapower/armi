...
class InjectInputs(EntryPoint):
    ...
    def addOptions(self):
        ...
        self.parser.add_argument("--geom", help="Path to geometry file", type=str, default=None)
        self.parser.add_argument("--settings", help="Path to settings file", type=str, default=None)

    def invoke(self):
        from armi.bookkeeping.db.database import Database

        if all(li is None for li in [self.args.blueprints, self.args.geom, self.args.settings]):
