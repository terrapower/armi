...
class Block(composites.Composite):
    ...
    def getSmearDensity(self, cold=True):
        ...
        if not circles:
            raise ValueError("Cannot get smear density of {}. There are no circular components.".format(self))
        clads = set(self.getComponents(Flags.CLAD)).intersection(set(circles))
        if not clads:
            raise ValueError("Cannot get smear density of {}. There are no clad components.".format(self))
    ...
    def setHeight(self, modifiedHeight, conserveMass=False, adjustList=None):
        ...
        if modifiedHeight < 0.0:
            raise ValueError("Cannot set height of block {} to height of {} cm".format(self, modifiedHeight))
...
class HexBlock(Block):
    ...
    def verifyBlockDims(self):
        ...
        try:
            ...
            runLog.info(
                "Block design {} is too complicated to verify dimensions. Make sure they are correct!".format(self)
            )
            return
        # check wire wrap in contact with clad
        if self.getComponent(Flags.CLAD) is not None and self.getComponent(Flags.WIRE) is not None:
    ...
    def getRotationNum(self) -> int:
        ...
        return np.rint(self.p.orientation[2] / 360.0 * 6) % 6  # assume rotation only in Z
    ...
    def hasPinPitch(self):
        """Return True if the block has enough information to calculate pin pitch."""
        return (self.getComponent(Flags.CLAD) is not None) and (self.getComponent(Flags.WIRE) is not None)
    ...
    def getPinPitch(self, cold=False):
        ...
        try:
            ...
            raise ValueError(
                "Block {} has multiple clad and wire components, so pin pitch is not well-defined.".format(self)
            )

        if wire and clad:
            return clad.getDimension("od", cold=cold) + wire.getDimension("od", cold=cold)
        else:
            raise ValueError("Cannot get pin pitch in {} because it does not have a wire and a clad".format(self))
