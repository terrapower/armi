...
class Assembly(composites.Composite):
    ...
    def scaleParamsToNewSymmetryFactor(self, oldSymmetryFactor):
        ...
        volIntegratedParamsToScale = self.getBlocks()[0].p.paramDefs.atLocation(ParamLocation.VOLUME_INTEGRATED)
    ...
    def getArea(self):
        ...
        try:
            ...
            runLog.warning("{} has no blocks and therefore no area. Assuming 1.0".format(self))
            return 1.0
        ...
        for i, b in enumerate(self):
            ...
            if refB.getHeight() == b.getHeight():
                ...
                while abs(heightChopped - heightToChop) > 1e-5:  # stop when they are equal. floating point.
                    ...
                    newBlocks += 1
                    runLog.important("Added a new block {0} of height {1}".format(newB, newB.getHeight()))
                    runLog.important("Chopped {0} of {1}".format(heightChopped, heightToChop))
                newBlocks -= 1  # subtract one because we eliminated the original b completely.
    ...
    def getFirstBlockByType(self, typeName):
        bs = [b for b in self.getChildren(deep=False) if isinstance(b, blocks.Block) and b.getType() == typeName]
        if bs:
            return bs[0]
        return None
