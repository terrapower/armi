...
# """manually load detailedAxialExpansion test reactor and ensure mass conservation for uniform mesh"""
# from armi.reactor.blueprints.reactorBlueprint import summarizeMaterialData
# filePath = os.path.join(TEST_ROOT, "detailedAxialExpansion")
# fileName = "armiRun.yaml"
# # instantiate case settings, blueprints, reactor, core & grid designs
# cs = settings.Settings(fName=os.path.join(filePath, fileName))
# cs = cs.modified(newSettings={"detailedAxialExpansion": False})
# bp = blueprints.loadFromCs(cs)
# r = reactors.Reactor(cs.caseTitle, bp)
# coreDesign = bp.systemDesigns["core"]
# gridDesign = bp.gridDesigns.get(coreDesign.gridName, None)
# core = coreDesign._resolveSystemType(coreDesign.typ)(coreDesign.name)
# core.setOptionsFromCs(cs)
# spatialGrid = gridDesign.construct()
# core.spatialGrid = spatialGrid
# core.spatialGrid.armiObject = core
# r.add(core)
# # add assemblies to core
# for locationInfo, aTypeID in gridDesign.gridContents.items():
#     newAssembly = bp.constructAssem(cs, specifier=aTypeID)
#     i, j = locationInfo
#     loc = core.spatialGrid[i, j, 0]
#     core.add(newAssembly, loc)
# summarizeMaterialData(core)
# coreDesign._modifyGeometry(core, gridDesign)
# # calculate the uniform mesh
# uniformMeshCore = core.getCoreWideUniformMesh(
#     Flags.FUEL, Flags.CONTROL
# )
# # apply the uniform mesh and ensure
# for assem in core.getAssemblies():
#     uniAssem = reactors.updateAssemblyAxialMesh(assem, uniformMeshCore)
#     self.assertAlmostEqual(
#         assem.getMass(),
#         uniAssem.getMass(),
#         msg="Assembly = {0} -- Mass is not being conserved between original and uniform axial mesh assemblies!".format(assem)
#     )
#     spatialLocator = assem.spatialLocator
#     core.removeAssembly(assem, discharge=False)
#     core.add(uniAssem, spatialLocator)
