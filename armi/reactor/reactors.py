...
class Core(composites.Composite):
    ...
    def updateUniformMesh(
        self, assemAxialMesh: list, currentUniformMesh: list, preserve: bool = False
    ):
        ...
        while ib < len(currentUniformMesh):
            ...
            if 0 < diff < 1:
                ...
                meshBetweenBlk = self.getBlockBoundsBetweenElevation(
                    assemAxialMesh,
    ...
    def createUniformMeshCore(
        ...
        for assem in secondaryAssemsToPreserve:
            uniformMeshSecondaryPreserve = self.updateUniformMesh(
    ...
    def getCoreWideUniformMesh(
        ...
        Parameters
        ----------
        primaryFlag: :py:class:`Flags <armi.reactor.flags.Flags` object.
        ...
        otherAssems = []
        for a in self.getAssemblies():
            if a.hasFlags(primaryFlag):
                primaryAssems.append(a)
            elif a.hasFlags(secondaryFlag):
        ...
        uniformMesh = self.createUniformMeshCore(
    ...
    def applyCoreWideUniformMesh(self, uniformMesh):
        ...
        for assem in self.getAssemblies():
            ...
            if (assem.getMass() - uniAssem.getMass()) > 1e-4:
                print(assem.getMass())
                print(uniAssem.getMass())
                raise RuntimeError(
                    "mass is not being conserved between assem and uniAssem!"
                )
            spatialLocator = assem.spatialLocator
            self.removeAssembly(assem, discharge=False)
...
def updateAssemblyAxialMesh(sourceAssembly, uniMesh):
    ...
    zLower = 0
    for i, zUpper in enumerate(uniMesh):
        overlappingBlockInfo = sourceAssembly.getBlocksBetweenElevations(zLower, zUpper)
        ...
        b = createNewBlock(overlappingBlockInfo)
        b.p.assemNum = uniAssem.p.assemNum
        b.name = b.makeName(uniAssem.p.assemNum, i)
        b.p.height = (
            zUpper - zLower
        )  # other z params get set in uniAssem.calculateZCoords
        uniAssem.add(b)
        zLower = zUpper
