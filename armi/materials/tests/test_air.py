...
"""
Reference thermal [hysical properties from Table A.4 in Incropera, Frank P.,
...
class Test_Air(unittest.TestCase):
    ...
    def test_density(self):
        ...
        for Tk, densKgPerM3 in zip(
            REFERENCE_Tk, REFERENCE_DENSITY_KG_PER_M3):
            if Tk < 2400:
    ...
    def test_heatCapacity(self):
        ...
        for Tk, heatCapacity in zip(
            REFERENCE_Tk, REFERENCE_HEAT_CAPACITY_kJ_PER_KG_K):
            if Tk < 1300:
                error = math.fabs((air.heatCapacity(Tk=Tk) - heatCapacity*1e+3)/(heatCapacity*1e+3))
                self.assertLess(error, 1e-2)
    ...
    def test_thermalConductivity(self):
        ...
        for Tk, thermalConductivity in zip(
            ...
            if 200 < Tk and Tk < 850:
                error = math.fabs((air.thermalConductivity(Tk=Tk) - thermalConductivity*1e-3)/(thermalConductivity*1e-3))
                self.assertLess(error, 1e-2)
