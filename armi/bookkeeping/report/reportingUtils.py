...
def _setGeneralCoreDesignData(cs, coreDesignTable):
    ...
    report.setData("Geometry File", "{}".format(cs["geomFile"]), coreDesignTable, report.DESIGN)
    report.setData(
        "Loading File",
...
def makeCoreAndAssemblyMaps(r, cs, generateFullCoreMap=False, showBlockAxMesh=True):
    ...
    core = r.core
    for plotNum, assemBatch in enumerate(iterables.chunk(assemsInCore, MAX_ASSEMS_PER_ASSEM_PLOT), start=1):
