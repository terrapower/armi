...
class Database:
    ...
    def open(self):
        if self.h5db is not None:
            raise ValueError(
                "This database is already open; make sure to close it before trying to open it again."
            )
    ...
    def splitDatabase(
        ...
        with h5py.File(backupDBPath, "r") as dbIn:
            ...
            if not set(keepTimeSteps).issubset(timeSteps):
                raise ValueError(
                    "Not all desired time steps ({}) are even present in the database".format(
                        keepTimeSteps
                    )
                )
        ...
        for paramDef in c.p.paramDefs.toWriteToDB():
            ...
            try:
                if paramDef.name in g:
                    raise ValueError(
                        "`{}` was already in `{}`. This time node should have been empty".format(
                            paramDef.name, g
                        )
                    )
                ...
                runLog.error(
                    "Failed to write {} to database. Data: {}".format(
                        paramDef.name, data
                    )
                )
                raise
    ...
    def getHistoriesByLocation(
        ...
        if len(objectTypes) != 1:
            raise TypeError(
                "The passed objects must be the same type; got objects of types `{}`".format(
                    objectTypes
                )
            )
    ...
    def _writeAttrs(obj, group, attrs):
        ...
        for key, value in attrs.items():
            try:
                ...
                runLog.info(
                    "Storing attribute `{}` for `{}` into it's own dataset within `{}/attrs`".format(
                        key, obj, group
                    )
                )
    ...
    def getCycleNodeAtTime(dbPath, startTime, endTime, errorIfNotExactlyOne=True):
        ...
        assert (
            endTime >= startTime
        ), f"The end time ({endTime}) is not greater than the start time ({startTime})."
        ...
        if not cycleNodes:
            raise ValueError(
                f"Provided start time ({startTime}) was greater than the modeled period: {thisTime}."
            )
