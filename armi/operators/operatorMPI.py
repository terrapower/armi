        ...
        if context.MPI_RANK == 0:
            ...
            try:
                ...
                if context.MPI_SIZE > 0:
                    ...
                    context.MPI_COMM.bcast("quit", root=0)  # send the quit command to the workers.
                    runLog.debug("Waiting for all nodes to close down")
                    context.MPI_COMM.bcast("finished", root=0)  # wait until they're done cleaning up.
                    runLog.important("All worker nodes stopped.")
                time.sleep(1)  # even though we waited, still need more time to close stdout.
