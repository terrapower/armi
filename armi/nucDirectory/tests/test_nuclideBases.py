...
class TestNuclideBases(unittest.TestCase):
    ...
    def test_nucBase_AllAbundancesAddToOne(self):
        for zz in range(1, 102):
            ...
            self.assertTrue(
                len(nuclides) > 0, msg=f"z={zz} unexpectedly has no nuclides"
            )
    ...
    def test_nucBasesImposeBurnChainTransmBulkStats(self):
        ...
        Exception: We allow 3e-4 threshold to account for ternary fissions, which are usually
        < 2e-4 per fission.
        """
    ...
    def test_nucBases_imposeBurn_nuSF(self):
        ...
        """
        actual = {
            nn.name: nn.nuSF for nn in self.nuclideBases.where(lambda nn: nn.nuSF > 0.0)
        }
        expected = {
            "CM248": 3.1610,
            "BK249": 3.4000,
    ...
    def test_getEndfMatNum(self):
        ...
        self.assertEqual(
            self.nuclideBases.byName["AM242"].getEndfMatNum(), "9547"
        )  # meta 1
    ...
    def test_curieDefinitionWithRa226(self):
        ...
        Notes
        -----
        The original definition of 1 Ci was based on the half-life of Ra-226 for 1 gram. The latest
        evaluations show that 1 gram is defined as 0.988 Ci.
        """
    ...
    def test_loadMcc3EndfVII0Data(self):
        """Tests consistency with the `mcc-nuclides.yaml` input and the ENDF/B-VII.0 nuclides in the
        data model.
        ...
        self.assertEqual(
            len(self.nuclideBases.byMcc3IdEndfbVII0), len(expectedNuclides) - 1
        )

    def test_loadMcc3EndfVII1Data(self):
        """Tests consistency with the `mcc-nuclides.yaml` input and the ENDF/B-VII.1 nuclides in the
        data model.
        ...
        # Subtract 1 nuclide due to DUMP2.
        self.assertEqual(
            len(self.nuclideBases.byMcc3IdEndfbVII1), len(expectedNuclides) - 1
        )
