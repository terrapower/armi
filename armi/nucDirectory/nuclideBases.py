...
r"""
This module provides fundamental nuclide information to be used throughout the framework and
applications.

TODO: JOHN: This will have to be updated significantly.
...
# Used to prevent multiple applications of burn chains, which would snowball unphysically.
# This is a bit of a crutch for the global state that is the nuclide directory.
burnChainImposed = False

instances = []
# The elements must be imported after the instances list is established to allow for simutaneous
...
class NuclideBase(INuclide, IMcnpNuclide):
    ...
    .. impl:: Isotopes and isomers can be queried by name and label.
        ...
        The :py:class:`NuclideBase <armi.nucDirectory.nuclideBases.NuclideBase>` class provides a
        data structure for information about a single nuclide, including the atom number, atomic
        weight, element, isomeric state, half-life, and name. The class contains static methods for
        creating an internal ARMI name or label for a nuclide. There are instance methods for
        generating the nuclide ID for external codes, e.g. MCNP or Serpent, and retrieving the
        nuclide ID for MC\ :sup:`2`-2 or MC\ :sup:`2`-3. There are also instance methods for
...
# TODO: JOHN: This needs to stay for now. (While it is still in the INuclide constructor.)
...
class NuclideBases:
    ...
    def factory(self):
        ...
        if len(self.instances) != 0:
            raise RuntimeError(
                "Nuclides are already initialized and cannot be re-initialized unless "
                "`nuclideBases.clear()` is called first."
            )
    ...
    def addNuclide(self, nuclide: INuclide):
        ...
        if isinstance(nuclide, IMcnpNuclide):
            if nuclide.getMcnpId() in self.byMcnpId:
                raise ValueError(
                    f"{nuclide} with McnpId {nuclide.getMcnpId()} has already been added."
                )
    ...
    @staticmethod
    def _failOnMissingActiveNuclides(missingActiveNuclides: set):
        """Raise ValueError with notification of which nuclides to include in the burn-chain."""
        msg = "Missing active nuclides in loading file. Add the following nuclides:"
        for i, nucList in enumerate(missingActiveNuclides, 1):
            msg += "\n {} - ".format(i)  # Index of
    ...
    def imposeBurnChain(self, burnChainStream):
        ...
        -----
        You cannot impose a burn chain twice. Doing so would require that you clean out the
        transmutations and decays from all the module-level nuclide bases, which generally requires
        that you rebuild them. But rebuilding those is not an option because some of them get set as
        class-level attributes and would be orphaned. If a need to change burn chains mid-run
        re-arises, then a better nuclideBase-level burnchain cleanup should be implemented so the
        objects don't have to change identity.

        Notes
        -----
        We believe the transmutation information would probably be better stored on a less
        fundamental place (e.g. not on the NuclideBase).
    ...
    def addNuclideBases(self):
        ...
        .. impl:: Separating natural abundance data from code.
            ...
            atomic number, excited state, element symbol, atomic mass, natural abundance, half-life,
    ...
    def readMCCNuclideData(self):
        ...
        .. impl:: Separating MCC data from code.
            ...
            contains the MC\ :sup:`2`-2 ID (from ENDF/B-V.2) and MC\ :sup:`2`-3 ID (from
            ENDF/B-VII.0) for all nuclides in MC\ :sup:`2`. The ``mcc2id``, ``mcc3idEndfVII0``, and
            ``mcc3idEndfVII1`` attributes of each :py:class:`NuclideBase
            <armi.nucDirectory.nuclideBases.NuclideBase>` instance are updated as the data is read,
    ...
    def updateNuclideBasesForSpecialCases(self):
        ...
        .. impl:: The special case name Am242g is supported.
            ...
            This function updates the keys for the :py:class:`NuclideBase
            <armi.nucDirectory.nuclideBases.NuclideBase>` instances for Am-242m and Am-242 in the
            ``byName`` and ``byDBName`` dictionaries. This function associates the more common
            isomer Am-242m by the name "AM242", and uses "AM242G" to denote the ground state.

        Notes
        -----
        This function is specifically added to change the definition of `AM242` to refer to its
        metastable isomer, `AM242M` by default. `AM242M` is most common isomer of `AM242` and is
        typically the desired isomer when being requested rather than than the ground state (i.e.,
        S=0) of `AM242`.
        """
        # Change the name of `AM242` to specific represent its ground state.
