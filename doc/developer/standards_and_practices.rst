...
===============================================
ARMI is used by TerraPower, LLC and other organizations to perform nuclear reactor simulation or analysis. Because of this, ARMI maintains a strong quality program that requires a human to be held responsible for all changes to the codebase. Because an LLM cannot be held responsible or accountable for quality or safety, ARMI contributions must adhere to the following guidelines:

1. All code and comments generated by an LLM shall be reviewed and edited for conciseness and correctness by a human before submitting a GitHub PR or Issue.
2. Documentation for code generated by an LLM shall be written by a human. Documentation generated by an LLM will not be
   accepted.
3. Tests for code generated by an LLM shall be written by a human, with an exception made for PRs expanding test
   coverage submitted by a qualified developer. For all other cases tests generated by an LLM will not be accepted.
4. PR and Issue titles and descriptions shall be written by a human. Agentic PRs or Issues will not be accepted.
