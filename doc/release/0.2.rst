...
#. Removed the last usage of ``settingsRules``, so that everything can use ``settingsValidation`` now.
#. TBD
...
#. Fixed bug where a settings validator would complain that both simple and detailed cycles inputs were being used even if only the detailed input was entered.
#. Fixed a bug where `getReactionRates()` was not accounting for burnup-dependent cross-sections. This changes the signature for the `getReactionRateDict()` function.
#. TBD
