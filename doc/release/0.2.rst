...
What's new in ARMI
------------------
#. Copy physics kernel inputs and outputs to directories organized by cycle and time step number (e.g., c2n1) to retain all important files from a simulation and avoid overwriting them. Changed the `dumpSnapshot` setting name to `runDetailedSnapshot`. `dumpSnapshot` is now used to specify the statepoints at which the physics kernel I/O is to be dumped. 
