...
What's new in ARMI
------------------
#. The method ``Material.density3`` is now called ``density``, and the old ``density`` is now called ``pseudoDensity``. (`PR#1163 <https://github.com/terrapower/armi/pull/1999>`_)

Bug fixes
---------
#. Fixed an invalid assumption on the lattice physics interface when using tight coupling for snapshot runs. (`PR#1206 <https://github.com/terrapower/armi/pull/1206>`_)
#. Fixed TBD
